...
const bash = events.filter((e) => e.type === "tool_execution_start" && e.toolName === "bash").length;

// Время по меткам событий: стена, а не сумма ходов. Простой между ходами - тоже цена модели.
const stamps = events.map((e) => Date.parse(e.at)).filter((n) => Number.isFinite(n));
...
// только сам вызов.
const verdicts = [...JSON.stringify(events).matchAll(/вердикт «([^»]+)»/g)].map((m) => m[1]);
