...
for model in "$@"; do
	...
	# Адвокату задаётся та же модель, что меряется. По умолчанию профиль говорит «наследовать», то
	# есть модель pi по умолчанию, - и адвокат тянет её вместо измеряемой. Роутер держит одну модель
	# за раз, поэтому каждый вызов адвоката оборачивался подкачкой туда и обратно: для моделей с
	# системного диска это секунды, для читаемых с архивного HDD - пять-шесть минут на каждую.
	if [ -f "$tree/.volna/project.md" ]; then
		sed -i "s|^- модель адвоката: .*|- модель адвоката: $model|" "$tree/.volna/project.md"
		grep -q "^- модель адвоката: $model" "$tree/.volna/project.md" && echo "адвокат: $model"
	fi
