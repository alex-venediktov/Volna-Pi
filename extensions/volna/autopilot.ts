...
export function partPrompt(input: PartPromptInput): string {
	...
	return [
		...
		[
			...
			`Лог итераций (${displayPath(input.volnaDir, paths.log(input.task))}) не читается вовсе - ни целиком, ни поиском: он append-only, в него пишет volna_journal. Это история всех частей вместе с отвергнутыми подходами, и в чужом контексте брошенная гипотеза читается как факт. Всё нужное о задаче есть в «Состоянии».`,
			`Накопленное знание лежит в вике ${displayPath(input.volnaDir, volnaPaths(input.volnaDir).wikiDir)}, а не в логе: это каталог файлов, читается обычным поиском (grep/find) и чтением файла.`,
			"Инструмент, у которого в имени есть wiki или knowledge, - чужая база знаний и к этому проекту отношения не имеет: ответа про этот проект в нём нет ни при каком запросе.",
		].join("\n"),
		[
...
/**
 * Сколько одинаковых вызовов подряд считать затором. Пять - уже патология: повторить одну команду
 * дважды бывает осмысленно (перечитать после правки), пять раз - нет. Порог низкий намеренно:
 * ошибочное прерывание стоит одного хода, а незамеченный круг - часа работы.
 */
...
export async function runAutopilot(options: AutopilotOptions): Promise<AutopilotReport> {
	...
	for (const [index, part] of queue.entries()) {
		...
		const session = startRpcSession({
			...
			watch: ({ toolCalls, elapsedMs, repeats, lastCall }) => {
				if (syncing) return toolCalls > SYNC_CALLS ? "сведение журнала затянулось" : null;
				// Повтор одного вызова подряд - не молчание и не перерасход, поэтому его не ловит ни
				// сторож простоя, ни потолок вызовов: сессия говорит, счётчик растёт медленно, а
				// работа стоит. Замер: пресет модели повторил одно чтение схемы вики 47 раз за 75
				// минут и до потолка так и не дошёл.
				if (repeats >= REPEAT_LIMIT) return `один и тот же вызов ${repeats} раз подряд: ${lastCall.slice(0, 80)}`;
...
export function capturePrompt(volnaDir: string, task: string): string {
	...
	return [
		...
		[
			`Записи по отдельным частям уже лежат в ${displayPath(volnaDir, paths.wikiDir)}: прочти их обычным поиском по файлам, чтобы не писать то же во второй раз.`,
			"Инструмент, у которого в имени есть wiki или knowledge, - чужая база знаний и к этому проекту отношения не имеет.",
		...
		`Лог итераций (${displayPath(volnaDir, paths.log(task))}) не читается: итоги частей уже выше, а остальное там - ход работы вместе с отвергнутыми подходами.`,
