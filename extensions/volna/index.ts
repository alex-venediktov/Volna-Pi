...
/**
 * Отказ читать лог итераций. Запрет держится устройством, а не просьбой в промпте: просьбу сессия
 * видит один раз в начале хода, а тянется к логу тогда, когда уже потеряла нить - и читает историю
 * всех частей вместе с отвергнутыми подходами. В чужом контексте брошенная гипотеза читается как
 * факт о проекте, и стоит это дороже, чем неудобство отказа.
 *
 * Послабление для адресного поиска стояло здесь раньше и себя не оправдало: сессия вытаскивала им
 * десятки строк за раз и набирала ту же историю, просто порциями.
 */
function refuseLogRead(path: string, active: ActiveTask): { block: true; reason: string } {
	return {
		block: true,
		reason: [
			`Волна: лог итераций не читается (${path}) - ни целиком, ни поиском.`,
			`Картина задачи - секция «Состояние» в ${displayPath(active.volnaDir, active.journalPath)}: по ней задача и восстанавливается, и другого источника у тебя нет.`,
			"Лог append-only: в него пишет volna_journal, а читает его только сам флоу - там история всех частей",
			"вместе с отвергнутыми подходами, и в чужом контексте брошенная гипотеза читается как факт.",
		].join(" "),
	};
}
