 ...
 *
 * Копию гейт не ловит: `cp лог x && cat x` читает уже не лог. Гнаться за этим нет смысла - цель не
 * запретить доступ, а не дать истории всех частей осесть в контексте по случайности. Кто копирует
 * файл ради обхода, делает это намеренно и знает, зачем.
 */
