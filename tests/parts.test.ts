...
export async function run(): Promise<void> {
	...
	// Примечание пишется прозой и само содержит слова статуса. Разбор с конца превращал закрытую
	// часть в не начатую, а весь итог - в название, и тихо: список на месте и читается.
	const noisy = parsePartsText("9. патрули - сделано (2026-09-20, 1.5ч), закрыто, коммит f4b0dc7 - не начата")[0];
	check("статус берётся у первого совпадения, а не у последнего", noisy.status === "сделано", noisy.status);
	check("название не съедено примечанием", noisy.title === "патрули", noisy.title);
	check("примечание сохранено целиком", noisy.note.includes("f4b0dc7") && noisy.note.includes("не начата"), noisy.note);
	const plain = parsePartsText("1. схема хранения - сделано (2026-08-16, 3ч)")[0];
	check("скобки обычного примечания снимаются", plain.note === "2026-08-16, 3ч", plain.note);
	check("слово статуса внутри названия статусом не считается", parsePartsText("1. отчёт о сделанном")[0].status === "не начата");
